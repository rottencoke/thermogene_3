...
export async function sort_results_by_blast_param(blast_param, order) {
    ...
    if (arr_blastn_result_id.length) {
        ...
        for (const [index, id] of arr_blastn_result_id.entries()) {
            ...
            arr_blast_result_indexed.push({
                index,
                id: id,
                blast_param: blastn_result[blast_param].toString() // evalue用にtoString()にしてる
            });
        };
    }
    /// tblastnの場合
        ...
        for (const [index, id] of arr_tblastn_result_id.entries()) {
            ...
            arr_blast_result_indexed.push({
                index,
                id: id,
                blast_param: tblastn_result[blast_param].toString() // evalue用にtoString()にしてる
            });
        };
    }
    ...
    /// 降順
    if (order == "descending") {
        arr_blast_result_indexed.sort((a, b) => b.blast_param - a.blast_param);
    }
    /// 昇順
    else if (order == "ascending") {
        arr_blast_result_indexed.sort((a, b) => a.blast_param - b.blast_param);
    }

    // blastn
    if (arr_blastn_result_id.length) {
        // blastn_result_indexedのidを取り出して配列化
        arr_blastn_result_id_sorted = arr_blast_result_indexed.map(obj => obj.id);
    }
    // tblastn
    else if (arr_tblastn_result_id.length) {
        // tblastn_result_indexedのidを取り出して配列化
        arr_tblastn_result_id_sorted = arr_blast_result_indexed.map(obj => obj.id);
    }

    // ソートされたインデックスを使用してtempura_idを並び替え
    arr_tempura_id_sorted = arr_blast_result_indexed.map(item => arr_tempura_id[item.index]);
